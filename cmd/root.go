...
This program must be run as root or with sudo.`,
		PersistentPreRunE: func(cmd *cobra.Command, args []string) error {
			...
			v.BindPFlag("zig_only", cmd.PersistentFlags().Lookup("zig-only"))
			v.BindPFlag("zls_only", cmd.PersistentFlags().Lookup("zls-only"))
			v.BindPFlag("verbose", cmd.PersistentFlags().Lookup("verbose"))
			v.BindPFlag("no_color", cmd.PersistentFlags().Lookup("no-color"))
			v.BindPFlag("generate_env", cmd.PersistentFlags().Lookup("generate-env"))
			v.BindPFlag("show_settings", cmd.PersistentFlags().Lookup("settings"))
			v.BindPFlag("log_file", cmd.PersistentFlags().Lookup("log-file"))
			v.BindPFlag("enable_log", cmd.PersistentFlags().Lookup("enable-log"))
